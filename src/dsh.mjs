...
export function apply(ctx) {
  ...
  function instance(agent) {
    ...
    if (existing) { existing.approvals.close(); existing.disposeTools(); void existing.service.close(); owners.delete(agent.id); }
    ...
    const record = { agent, service, approvals, disposeTools: () => { for (const dispose of disposers.values()) dispose(); disposers.clear(); } };
    async function syncTools() {
      for (const spec of await savedTools(service)) if (!disposers.has(spec.name) && owners.get(agent.id) === record) disposers.set(spec.name, register(spec, agent.ctx.tools));
    }
    ...
    service.changes.add(syncTools); record.ready = syncTools();
    agent.ctx?.effect(() => () => { if (owners.get(agent.id) === record) owners.delete(agent.id); record.disposeTools(); approvals.close(); void service.close(); }, 'routinekit: discard task-local capture and close browser');
    return record;
  }
  ...
  ctx.on('agent/created', async ({ agent }) => { await instance(agent).ready; });
  function register(spec, registry) { return registry.register({
    ...
    async execute(args, exec) {
      const owner = instance(exec.agent); await owner.ready; const { service } = owner;
  ...
  for (const spec of TOOL_SPECS) register(spec, ctx.tools);
  ...
  ctx.effect(() => () => {
    for (const record of owners.values()) { record.disposeTools(); record.approvals.close(); void record.service.close(); }
